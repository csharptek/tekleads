...
export default function HRWhatsAppInboxView() {
  ...
  const loadInbox = async () => {
    try {
      const data = await api.get<Thread[]>("/api/whatsapp/inbox?inbox=hr");
      setThreads(data || []);
    } catch { }
    setLoading(false);
  };
      ...
      <div style={{ width: 300, minWidth: 240, borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", background: "var(--surface)" }}>
        <div style={{ padding: "16px 16px 12px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <div className="page-sub" style={{ fontSize: 11 }}>{threads.length} conversations · +91 numbers</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          {!loading && threads.length === 0 && (
            <div style={{ padding: 24, textAlign: "center", color: "var(--muted)", fontSize: 13 }}>
              No HR messages yet.<br />Replies from +91 numbers appear here.
            </div>
          )}
          {threads.map(t => (
            <div key={t.phone}
              ...
              <div style={{ marginTop: 4, display: "flex", gap: 6, alignItems: "center" }}>
                ...
                {t.unreadCount > 0 && (
                  <span style={{ background: "#f59e0b", color: "white", borderRadius: 999, fontSize: 10, padding: "1px 6px", fontWeight: 700 }}>
                    {t.unreadCount} reply
                  </span>
                )}
              </div>
        ...
        {!selected ? (
          ...
          <>
            <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 12, background: "var(--surface)" }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#f59e0b", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              {messages.map(m => {
                ...
                return (
                  <div key={m.id} style={{ display: "flex", justifyContent: isOut ? "flex-end" : "flex-start" }}>
                    <div style={{
                      maxWidth: "70%", background: isOut ? "#fde68a" : "white",
            ...
            <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border)", background: "var(--surface)" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  ...
                  <button className="btn btn-sm" onClick={() => sendReply("template")} disabled={sending}
                    style={{ background: "#b45309", color: "white", border: "none", fontSize: 11 }}
                    title="Send template">
                    Template
                  </button>
                </div>
              ...
              <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 6 }}>
                HR inbox — replies from +91 numbers. Free-form text only within 24hr window.
              </div>
            </div>
          </>
        )}
