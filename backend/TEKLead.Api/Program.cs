...
builder.Services.AddScoped<JdQualityService>();
builder.Services.AddScoped<ArtifactChatService>();
...
builder.Services.AddHostedService<JobLeadEmailWorker>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    try { var jdSvc = scope.ServiceProvider.GetRequiredService<JdQualityService>(); await jdSvc.EnsureSchema(); } catch (Exception ex) { app.Logger.LogError(ex, "JdQuality schema failed"); }
    try { var artChatSvc = scope.ServiceProvider.GetRequiredService<ArtifactChatService>(); await artChatSvc.EnsureSchema(); } catch (Exception ex) { app.Logger.LogError(ex, "ArtifactChat schema failed"); }
